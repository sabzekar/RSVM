function [svm_struct, svIndex] = FESVMTrain(training, groupnames, varargin)
...
Alpha=.9;
C=1000;
%***********************Fuzzy Membership in my_f1*******************
membership1=mem_cal(x1);
membership2=mem_cal(x2);
membership=[membership1 membership2]; 
my_f1=[]; 
 for i=1:NofSample
    temp=C*(membership(i)-1);
    my_f1=[my_f1;temp];
 end
%*******************************************************************
%**********************importance degree FCM in my_f2****************
...
my_f=[my_f2 my_f1];  
%**************************************************************************
zer=zeros(NofSample,1);
zer=zer';
Aeq=[groupIndex' zer];
attach1=zeros(NofSample,NofSample);
H=[H;attach1];
attach2=zeros(2*NofSample,NofSample);
H=[H attach2];
UB=[C*ones(NofSample,1);Inf*ones(NofSample,1)];


        % X=QUADPROG(H,f,A,b,Aeq,beq,LB,UB,X0,opts)
        [alpha, fval, exitflag, ...
            output, lambda] = quadprog(H,-my_f,[],[],...
            Aeq,0,zeros(2*NofSample,1), ...
            UB, ones(2*NofSample,1), ...
            qp_opts); %#ok
