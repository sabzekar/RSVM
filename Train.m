function [svm_struct, svIndex] = Train(training, groupnames, varargin)
...
%   $Revision: 1.1.12.11 $  $Date: 2007/09/11 11:42:58 $
...
NofSample1=size(x1,1);
NofSample2=size(x2,1);
[center,U,obj_fcn] = fcm(x,cluster_n);
row1=U(1,:);
row2=U(2,:);
sum1=sum(row1(1:NofSample));
sum2=sum(row2(1:NofSample));
if (sum1>sum2)
    f_membership=[row1(1:NofSample1) row2(NofSample1+1:NofSample)];
else
    f_membership=[row2(1:NofSample1) row1(NofSample1+1:NofSample)];
end
f_membership= 1-f_membership;
...
% 
% for i=1:NofSample1
%     temp11=[temp11 norm(x1(i,:)-mean1)];
%     temp22=[temp22 norm(x1(i,:)-mean2)];
% end
% for i=1:NofSample2
%     temp12=[temp12 norm(x2(i,:)-mean1)];
%     temp21=[temp21 norm(x2(i,:)-mean2)];
% end
% rplusmin=min(temp12);
% temp11=max(temp11);
% rplusmax=max(temp11,r);
% temp21=max(temp21);
% rminusmax=max(r,temp21);
% rminusmin=min(temp22);
% f_membership=[];
% for i=1:NofSample1
%     temp=(1-(norm(x1(i,:)-mean2)/rminusmax+Mu))*(rminusmax+Mu/rminusmax+Mu+rminusmin);
%     f_membership=[f_membership temp];
% end
% for i=1:NofSample2
%     temp=(1-(norm(x2(i,:)-mean1)/rplusmax+Mu))*(rplusmax+Mu/rplusmax+Mu+rplusmin);
%     f_membership=[f_membership temp];
% end
% f_membership= 1-f_membership;

%**********************************************************************
% f_membership=.6*ones(NofSample,1);

my_f=[];       
 for i=1:NofSample
    temp=1-f_membership(i)+(Alpha*f_membership(i));
    my_f=[my_f;temp];
 end
% my_f=1.1*ones(NofSample,1);
        
        % X=QUADPROG(H,f,A,b,Aeq,beq,LB,UB,X0,opts)
        [alpha, fval, exitflag, ...
            output, lambda] = quadprog(H,-my_f,[],[],...
            groupIndex',0,zeros(nPoints,1), ...
            C*ones(nPoints,1), ones(nPoints,1), ...
            qp_opts); %#ok
